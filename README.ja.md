...
nudge の上限に達しているかもしれません(評価を挟まずに 5 回引き留められると、hook は fail-open に切り替わり `stalled` を記録します)。
一時停止ノートが消費された直後かもしれません。
`HEADSIGN_OBSERVER` が設定されているかもしれません。
しかも 1 回確かめるたびに上限を 1 つ消費するので、習慣にするものではなく、意図して使う探りです。

*セッション*の場合、同じ試し方はもっと弱くなります。
`Stop` が通すのは、駆動者でないと積極的に断定できる停止、つまり両方の識別子が解決できて食い違う場合だけです。
