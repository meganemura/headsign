...
止まったターン側と刻印側のどちらか一方でも識別子が解決できないとき、`Stop` は従来どおり催促します。
...
なお、この問い自体に到達しない場合が一つあります。
`headsign claim` によって駆動者が着席した run では、刻印されているのはエージェントの識別子なので、`Stop` は何も比較せず、すべてのセッションを無条件に通します。
どのセッションもそのエージェントではありえないからです([複数セッション](#複数セッション)を参照)。
...
判定を一切行わないためです。
`driver:` 行(`RUNNING` のときだけ表示されます)は、自セッションの解決済み識別子が刻印された driver と一致すれば `this session, or an agent it delegated to`、両方が解決できて食い違えば `another session`、どちらか一方でも解決できなければ `unknown` になります。
...
この最初の読みが冗長なのは意図的です。
委譲されたエージェントは spawn 元セッションの識別子を共有するため、一致から絞り込めるのは*そのセッションか、その配下のどれかのエージェントか*までで、そこで止まります。
そのうちのどれであるかまでは、互いに見分けられません。
`status` が比較しているのは環境が渡してくる識別子であり、あるセッションとその委譲先を区別できる識別子は存在しません。
その溝こそが `headsign claim` の存在理由です。

**あなたが委譲されたエージェントなら、ターンを終えて何が起きるかを見てください。**
**`headsign next` へ差し戻されたなら、その run はあなたが駆動すべきものです。**
`SubagentStop` がエージェントを引き留めるのは、刻印された駆動者と一致したときと、claim を seal するときだけです。
ですから、どちらのメッセージが返ってきたかを読み分けてください。
通常の催促はフェーズ名を挙げて `headsign next` の実行を促すもので、これはあなたが既に駆動者であることの確認です。
`Claim confirmed: this agent now drives …` はまったく別の意味です。
...
nudge の上限を使い切ったのかもしれません(評価もノートも挟まずに 5 回目に引き留められたターンは、引き留めたうえで `stalled` を記録し、それ以降の停止をすべて通します)。
一時停止ノートが消費された直後かもしれません。
`HEADSIGN_OBSERVER` が設定されているかもしれません。
探りはほぼ無償ですが、完全に無償ではありません。
通常の催促として返ってきた場合は上限を 1 つ消費し、あなたが駆動者で一時停止ノートを構えていた場合は、その探りがノートを消費してしまいます。
習慣にするのではなく、意図して使ってください。

*セッション*の場合、同じ試し方はもっと弱くなります。
所有者チェックが識別子を根拠に停止を除外するのは、両方の識別子が解決できて食い違う場合だけです。
そのため、識別子が一度も刻印されていない run では、そのディレクトリにいるすべてのセッションが、駆動者かどうかに関わらず催促されます。
...
claim を飛ばした代償は、紛らわしい表示欄だけにはとどまりません。
委譲されたエージェントの素の `headsign next` が spawn 元セッションを刻印してしまうと、その run が出す催促はすべて*そちらの*セッションへ飛びます。
そのセッションはたいてい、委譲の完了を待って手が空いたまま座っているだけです。
...
刻印された駆動者でもなく、構えられた claim marker のもとで最初に停止したわけでもないエージェントが、引き留められることはありません。
