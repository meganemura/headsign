...
- **質問は一つ、駆動者も一つ**:gate も、ダッシュボードもありません。`next` が判定を返し、失敗したときは落ちたチェックとその出力を、そのまま残作業リストとして返します。この質問はリポジトリの誰もが選べるメニューではなく、*駆動している*セッションだけの質問です。もう一つのコマンド `status` は観察者のためのものです。読み取り専用で、判定も遷移も一切行いません([複数セッション](#複数セッション)を参照)。`claim` も判定しません。run の主導権をわざと持ち替える、その一瞬のためだけに、Stop hook を介して driver の所有権をこのセッションへ渡します([複数セッション](#複数セッション)を参照)。
...
| `headsign claim` | Stop hook を介して driver の所有権をこのセッションに渡す。run の駆動を誰に任せるかを委譲するためのコマンド([複数セッション](#複数セッション)を参照) |
...
`headsign claim` による引き継ぎ(後述)のあとは、代わりに `driver: claimed` と表示されます。
`status` は、claimed な run について this session か another session かを意図的に推測しません。
そもそも `claim` を必要にしているのと同じ解決不能な溝が、その推測自体も信頼できなくしているからです。
`claimed` は、現在の driver がハンドシェイクによって着席したという事実だけを、正直に示します。
...
所有権は通常、直前の節で見たとおり、解決可能なセッション識別子とともに最後に `next` を呼んだセッションにそのまま従います。
ところが Claude Code の agent-teams 機能の中では、この識別子は*意図的な*引き継ぎの拠り所にできるほど信頼できません。
セッション自身の Bash tool の内側からは、「自分のセッション」と「自分を spawn したリードセッション」を区別する方法がありません。
そのため、teammate にただ `next` を呼び始めてもらうだけでは、誤ったセッションが黙って driver として刻印されかねません。
`headsign claim` はこれを避けるため、刻印の役目を Stop hook に譲ります。
Stop hook だけは、発火のたびに Claude Code から本物のセッション ID を渡されるので、*実際に*見分けがつく唯一の場所です。
手順は 2 拍です。

1. run を駆動させたいセッションから `headsign claim` を実行します。
   これは一発券のマーカーを構え、ターンを終えるよう伝えるだけで、それ自体は何も刻印しません。
2. そのターンを終えます。
   セッション ID を解決できた次の Stop hook の発火が、*その*セッションを driver として採用し(`.headsign/state.json` の `driver_session` / `driver_source`)、ブロックメッセージでそれを確認し、`.headsign/log` に `claimed` 行を記録します。
   その確認が届いたら、いつもどおり `headsign next` で run を駆動してください。

典型的な委譲はこう進みます。
「teammate、この run を頼む」→ teammate が `headsign claim` を実行して停止 → hook の確認メッセージが届く → teammate が `headsign next` で駆動を始める、という流れです。
この方法で claim した所有権は粘着性があります。
同じ共有環境にある無関係なセッションの、通常の `next` 呼び出しでは、素の env 刻印のように黙って奪い返されることはありません。

これは lock ではなくハンドシェイクです。
別のセッションがたまたま先に停止して誤って採用されてしまった場合は、正しいセッションからもう一度 `headsign claim` を実行してください。
新しい claim は常に勝ち、誤採用は次の停止で自己修復されます。
この狭いレース窓を含む仕組み全体は [ADR-0009](docs/adr/0009-claim-handshake.md) にあります。
...
| `HEADSIGN_OBSERVER` | あなた自身が明示的に | 空でない任意の値(慣習として `=1`)を設定すると、駆動者の所有権にかかわらず、そのセッションの停止が Stop hook を無条件に通過するようになります。自分がもっぱら観察しているだけだと分かっているセッション向けの手動 opt-out です。セッション識別子がまったく解決できない環境では特に有用です。 |
...
- **オーケストレーションはしません。** 1 run につきアクティブなフェーズは一つです。DAG、並列フェーズ、worktree 管理、プロバイダ抽象化、ペルソナ、テンプレートや式言語、MCP サーバー、TUI、run 横断のダッシュボードは持ちません。ハーネス側に賢さが必要になったら、それは賢さの置き場所が間違っています。
