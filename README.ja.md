...
| 意味 | 留まる | 出て、入り直す |
| `clear:` | 動かない | 動く |
| tree-hash キャッシュ | 効く | 効かない |
| 答えのトークン | `RETRY` | `ADVANCE` |

つまり自分自身へのルーティングは、そのフェーズが `clear:` に挙げた成果物を削除し、キャッシュされた判定も捨てます。
次の `headsign next` は前回の判定を再表示するのではなく、判定をやり直すということです。
...
- `when:` はフェーズの `env:` を引き継ぎ、任意で `timeout:`(秒、既定は 120)を取れます。チェックとまったく同じ扱いです。
...
ただし `claim` marker が構えられていて、そのターンが自分を名乗れる最初の委譲エージェントのものである場合は、そのエージェントが着席します([複数セッション](#複数セッション)を参照)。
...
刻印された駆動者でもなく、構えられた claim marker のもとで最初に自分を名乗ったわけでもないエージェントが、引き留められることはありません。
