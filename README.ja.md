...
**自分が駆動者かどうかを知る唯一の確実な方法は、ターンを終えてみて、停止境界の hook が `headsign next` へ差し戻してくるかどうかを見ることです。**
その hook は記録された駆動者だけをブロックし、それ以外の停止はすべてそのまま通します。
ですから、催促されること自体が答えです。
引き留められたなら、その run はあなたが駆動すべきものです。
run が `RUNNING` のままターンが静かに終わるなら、そうではありません。
