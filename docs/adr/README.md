...
| [0025](0025-a-stop-that-passed-and-a-stop-that-never-ran.md) | Telling a stop that passed from a hook that never ran — the `unheld` log event and a `last_stop` field written in the same locked write, why the already-continuing flag is not called a loop guard, the flagged check gains a body without ever reaching the adoption gate, and two `status` lines *(amends 0002, 0004, 0006, 0008)* |
