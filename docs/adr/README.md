...
| [0013](0013-claim-only-driver-identity.md) | Claim-only driver identity — retiring the environment stamp: why `Stop` now compares nothing, `driver_agent`, the two-valued `driver:` line, and what a single identity path gives up | *(its one-path decision's second clause and its `Stop`-compares-nothing decision retracted by [0027](0027-recording-who-drove-a-run.md))*
...
| [0017](0017-three-budgets-and-the-recoverable-ceiling.md) | Three budgets, one of which can fire on a healthy run — `limits.max_total_iterations` escalates without ending the run, the `ceiling` log event, and why the other two escalations stay terminal *(its wall-clock refusal clarified by [0031](0031-when-the-run-entered-the-phase.md))* |
...
| [0023](0023-pinning-the-graph-a-run-is-walking-under.md) | A run pins the rules it walks under — a fingerprint per reachable phase plus `limits`, `description` excluded; a change is reported once as an ESCALATE that ends nothing, restoring the file is free, asking again accepts and counts it, and `COMPLETE` says how many *(amends 0016 §5 and 0017; its §8 amended by [0029](0029-status-answers-for-the-file.md))* |
...
| [0031](0031-when-the-run-entered-the-phase.md) | When the run entered the phase — `state.json` gains `phase_entered_at`, stamped where `clear:` runs so a retry never moves it and a re-entry does, and `status` prints it verbatim while headsign still computes no duration *(amends 0017)* |
| [0032](0032-the-gate-says-how-far-it-got.md) | The gate says how far it got — one stderr line per check that produced an exit code, saying which way it went, after a first line naming how many the gate holds, so a caller that stops waiting can read where the gate stood; stdout keeps the token line untouched, and what an interrupted lap costs is answered in the reference *(amends 0002)* |
| [0033](0033-the-one-variable-headsign-sets.md) | The one variable headsign sets — `HEADSIGN_WORKFLOW_FILE` reaches a gate's checks, a `ready:` probe and an `on_pass` `when:`, carrying the path this run recorded rather than a normalised one; the line drawn is that headsign supplies only what it alone resolved and the author could not have written, which the phase name and the attempt count both fail *(amends 0014 §1)* |
| [0034](0034-a-record-is-one-line.md) | A record is one line, and `logLine` is what keeps it one — line breaks in author-written text (an abort reason typed with one, a check's or a phase's name) escaped on the finished line rather than at each producer, for reading rather than for reading back *(amends 0004)* |
