...
| `src/gate.ts` | run one phase's checks (shell, timeout, output tail), timing how long a failing one ran with a monotonic clock — outside ADR-0004's guarantee (that guarantee is about the wall-clock datetime that lands on disk, not this interval), since this module already touches the outside world; resolve which route of a list-form `on_pass` matched, by running its `when:` commands the same way (ADR-0011) | what a route target means, state, git |
