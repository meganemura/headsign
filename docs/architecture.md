...
| `src/gate.ts` | run one phase's checks (shell, timeout, output tail), timing how long a failing one ran with a monotonic clock — the one exception to ADR-0004's clock-stays-in-`cli.ts` rule, since this module already touches the outside world; resolve which route of a list-form `on_pass` matched, by running its `when:` commands the same way (ADR-0011) | what a route target means, state, git |
