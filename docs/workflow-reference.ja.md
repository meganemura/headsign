...
何らかの目的でプログラムが Claude Code を子プロセスとして起こすと、その子プロセスは run のディレクトリに立つセッションになり、ほかと同じように催促されます。
その子プロセスは催促が自分宛てでないと知らないため応えようとし、呼び出し元に返ってくるのは、起動した目的の出力ではなく headsign についての散文になります。
そのターン終了も、ほかのブロックされた停止と同じく、run の nudge 上限を 1 つ消費します。
