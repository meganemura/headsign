...
*別のチェックアウトの中* — 姉妹クローン、ドキュメント用のリポジトリ、たまたまセッションが立っている場所 — でも同じで、`cd` で途中から移ったのか最初からそこだったのかは関係ありません。
探索はそのリポジトリのルートで打ち切られ、run が見つからず、何も書きません。ログ行も `last stop:` も残りません。
