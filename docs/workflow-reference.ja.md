...
**落ちたゲートは、どの検査に届かなかったかを言います。**
検査は順に走り、ゲートは最初の失敗で止まるので、その後ろの検査は 1 本も走りません。そして**落ちた周は、そのゲートが最も薄く検査した周**です。
ブロックはそれを名指します —— `--- 2 of 3 checks ran; 1 not run: deferrals tracked ---`。同じ数は `.headsign/log` の retry 行にも `ran=2/3` として載るので、run が終わったあとでも答えられます。
