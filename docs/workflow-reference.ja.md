...
`SessionStart` は、セッションが作業を始める前に実行中のワークフローを知らせます。
ゲートは実行せず、run も変更しません。
残る二つの hook がバックストップを構成します。
`Stop` はセッション自身を受け持ちます。
`SubagentStop` は、そのセッションが run の駆動を委譲したエージェントを受け持ちます。
