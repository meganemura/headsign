...
ただしこの遡りは上方向にしか進みません。
この探索は上へしか進まず、しかも最初に出会った `.git` で打ち切られるので、外れ方が2通りあり、どちらも無言です。
モノレポのルートのように run のあるディレクトリより*上*でセッションが止まっていると、hook は run を見つけられません。
そして*別のチェックアウトの中* — 姉妹クローン、ドキュメント用のリポジトリ、`cd` したまま戻らなかった先 — で終わった場合は、探索がそのリポジトリのルートで打ち切られ、run が見つからず、何も書きません。
ログ行も `last stop:` も残らず、バックストップが入っていない状態と見分けがつきません。
