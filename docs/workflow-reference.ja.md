...
`RUNNING` の run には、これに加えて 2 行が出ることがあります。
...
静かに終わる 5 つの理由のうち、呼び出し側が*自分について*答えられる唯一のものでもあります。
解決すべき識別子が無いからです。

条件付きというのは、1 バイト単位で条件付きという意味です。
停止をまだ一度も処理していない run を、スイッチの無い環境で読めば、どちらの行も存在しなかったときの `status` の出力とまったく同じものが出ます。
どちらの行も判定ではありません。
...
ターンが静かに終わる理由は 5 つあり、それらを見分けるために見る場所は、理由ごとに違います。

| ターンが静かに終わった理由 | 見分け方 |
| --- | --- |
| Claude Code がそのターンをすでに再開させていた | `.headsign/log` の `unheld` 行。`headsign status` の `last stop:` 行 |
...
**`driver:` は 4 行目を絞り込むだけで、決着はつけません。**
...
**`unheld` 行が言うのは、*どこかの* stop hook がそのターンを引き留め、そのあと headsign が手を引いたということです。**
