...
ディスク上の状態があるおかげで、駆動していたセッションが消えても run は生き延びます。API の障害、コンテキストの枯渇、人が作業を止めた場合、どれでも同じです。
...
*セッション*が引き継いだ場合はそれができません。seal は `SubagentStop` でしか起きず(ADR-0010)、それはセッションのターン終了が発生させるイベントではないからです。
