...
どちらの行も、まずプロジェクト内のインストールを探し、次に `PATH` 上のものを探します。
最後に `node` を探します。
...
この代償はバックストップだけでなく発見の手段にも及びます: run が既にあるリポジトリを開いたセッションは、これまで催促されることでそれに気づいていました。
それは近くにいる全員を催促する副作用にすぎず、バックストップ本来の役目ではありません。
...
この規則は以前にも増して当てはまります。
他の誰かがすでに動かした run は、近くで止まった者を催促して自ら知らせてくれなくなりました。
そのため、その run は `headsign status` でしか見つけられません。
