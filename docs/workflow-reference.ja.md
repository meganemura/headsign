...
run の途中でも手順の修理が必要になることがあります。
指示が作らせない成果物をゲートが要求する場合は、作る側、読む側、保存期間を一緒に確認します。
検証作業にコード編集を必須とせず、案の却下にはやり直せる経路を用意します。
同じ失敗で試行回数を使い切る代わりに、依頼の権限内でこれらの契約を修理します。
レビューゲートは現在の最終判定と査読した成果物の対応を検査します。
文書の容量や過去の承認だけでは、この対応を確認できません。
履歴と現在の判定を分け、修理時には却下、入力の欠落、成果物の変更も検証します。
この対応は workflow の作者が定義し、CLI は指定された検査を実行します。

新しい run は、手順の改善を既定で促します。
`headsign start [name] [--workflow path] --no-optimize` で、その run だけ無効にできます。
名前と `--workflow` は、どちらか一方を指定します。
旧バージョンで開始した run に、改善用の識別子を後付けすることはありません。
アップグレードのために run を再起動する必要もありません。

`workflow` skill は運転中の観察を残し、`optimize` skill は終端で方法を評価します。
同じゲートの失敗が初めて繰り返された時に、作業と手順のどちらに修理が必要かを一度だけ尋ねます。
`COMPLETE` または run を終える `ESCALATE` では、skill 名と評価記録のパスを案内します。
既存の振り返りがあれば、その所見を再利用します。
編集を必須にせず、将来の結果に影響する修理、提案、現状維持、延期を選びます。

評価記録は `.headsign/optimization/<id>/assessment.md` に置きます。
このディレクトリは Git 管理外で、次の run を開始した後も残ります。
記録は 64 KiB 以下の通常ファイルとし、1 行目を `NO_CHANGE`、`APPLIED`、`PROPOSED`、`DEFERRED` のいずれかにします。
残りには空でない説明を書きます。
LF と CRLF を受け付けます。
記録は自己申告であり、改善の正しさや権限を証明しません。
有用な知見は、レビューした workflow、検査、通常の文書へ移します。
有効な記録ができると、終端の案内と追加継続の要求を止めます。
`status` は有効な識別子がある run の評価状態とパスを読み取り専用で表示します。

Stop と SubagentStop の追加要求は、担当セッションまたは担当エージェントを特定できた場合に限り、run 全体で一度だけです。
識別情報が不明な場合、読み取りやロックに失敗した場合は、停止を妨げません。
`stop_hook_active`、abort、無効化した run、既に要求済みの run も通します。
空でない stop note はその停止での要求を抑えます。
明示的な停止は優先し、可能なら skill が `DEFERRED` を記録します。
hook がユーザーの停止意図を入力から推測することはできません。

改善には現在の依頼の範囲と残り予算を使い、再帰的な改善 run を開始しません。
依頼の目的と制約を保つ可逆的な修理は、既存の権限で進められます。
成果の条件、明示された予算、アクセス、公開、外部への連絡を変えるには、それに対応する権限が必要です。
実行中のグラフ変更は報告を受けてから、別の呼び出しで `next --accept-graph-change` を実行します。
この操作は変更の明示的な受入れを記録し、権限を付与するものではありません。
完了後に成果物を編集した場合は再検証し、それ以前の `COMPLETE` は過去の判定として扱います。

詳細な設計判断は [ADR-0038](adr/0038-a-run-assesses-its-procedure.md) にあります。
...
キーの `headsign` はマーケットプレイスの名前です。
この名前は [`.claude-plugin/marketplace.json`](../.claude-plugin/marketplace.json) から来ています。
その `source` は、マーケットプレイスの場所を示します。
`enabledPlugins` のキーは `plugin@marketplace` の形です。
`headsign@headsign` は「`headsign` という名前のマーケットプレイスの中の、`headsign` という名前のプラグイン」です。
後者は [`plugin/.claude-plugin/plugin.json`](../plugin/.claude-plugin/plugin.json) から来ています。
このプロジェクトが同梱するプラグインは一つなので、両方に同じ語を使っています。
これは形式上の決まりではありません。

この二つのキーは headsign のものではなく Claude Code のものです。
そのため、以下ではこのファイルが*宣言*する内容を説明します。
各人の Claude Code が宣言をどう処理するかは、Claude Code 自身のドキュメントが説明します。
スキーマが変わったときも、そのドキュメントが変更に追随します。
このページはどこにも同梱されませんが、README のほうは npm のキャッシュや fork の中で凍りつきます。
そのため、変更される説明はこのページに置いています。
...
これにより、チーム全体でゲートのふるまいが統一されます。
ワークフローのスキーマは 1.0 より前です。
定義していないキーを拒否します([ADR-0015](adr/0015-strict-schema-and-version-0-1.md))。
そのため、固定していないチームでは、あるメンバーの手元で走るワークフローが、別のメンバーの手元では validate に通らない場合があります。
ハーネスの差が、仕事の差として現れます。
一方で、固定すると誰かが pin を更新し続ける必要がありますが、古くなった pin は表面に現れません。
固定したバージョンに無いものをワークフローが使うまで、どこもおかしく見えず、そのときには「三つ前のリリースにいる」ではなく「headsign が壊れている」という形で届きます。
同じエントリには `autoUpdate` というフラグもあります。
pin と自動更新は、同じ問いに対する逆向きの答えです。
どちらかが成り行きで決まるより、リポジトリが意図して片方を選ぶほうが良いはずです。
...
そこには、サードパーティのマーケットプレイスでは既定で auto-update が無効になり、利用者自身が更新を実行すると記録されています。
コミットしたファイルの `ref` を動かすと更新が始まりますが、更新はまだ完了しません。
...
Codex では 2 コマンドを使い、先にマーケットプレイスを更新します:
...
これだけを実行すると、そのスナップショット内のものを入れ直します。
スナップショットを取った後に公開されたリリースは見えず、**コマンドは成功して何も変わりません**。
`marketplace upgrade` はスナップショットを取り直します。
そのあとの `add` は新しいバージョンを見つけられます。
...
この形式は、[リポジトリ全体でプラグインを有効にする](#リポジトリ全体でプラグインを有効にする)で説明した `enabledPlugins` の形式と同じです。
セッションの中からなら、`/plugin` が同じ操作にメニュー経由で届きます。

どちらの経路でも、取得した複製はホストを再起動するまで使われません。
更新の適用は、宣言と取得に続く三つ目の操作です。
その再起動までは、`headsign version` が答えるのは実際に動いている複製、つまり古いほうです。
この答えはホストごとに出ます。
1 台のマシンに 2 つのホストが在れば、両方を再起動するまで答えが食い違います。

**プロジェクトが固定したバージョンは、run が使うバージョンとは限りません。** インストール済みのプラグインの複製はバージョンごとに分かれた場所に置かれます。
1 バージョンに 1 ディレクトリを使います。
そのパスは [maintenance.md](maintenance.md#live-patching-an-installed-plugin-local-testing) にあります。
そのため、プロジェクトが新しいリリースへ移っても、古い複製のふるまいは、その複製を更新するまで変わりません。
コミットしたファイルが述べるのはチームが欲しいバージョンで、そのマシンで `headsign next` が実際に走らせるのは、そのマシンにある複製です。
「修正が入っていない」「一人だけゲートのふるまいが違う」という報告が来たら、まず現在動いているバージョンを確認してください。
`headsign version` は、いま実際に動いている複製自身が答えます。
同じものを表示する `headsign --version` でも構いません。
この番号はバンドルのビルド時に焼き込まれ、実行時には `package.json` を読みません。
マーケットプレイスからキャッシュされた複製の上には `package.json` が無いからです。
その設計の理由は [ADR-0002](adr/0002-single-question-and-output-contract.md) にあります。
報告される番号とパッケージの番号を噛み合わせる仕組みも、同じ ADR にあります。
これまでの二つの手がかりも、引き続き持っておく価値があります。
それらは複製が*どこに*あり、どの経路で来たのかを示すため、二つの複製を識別できます。
インストール済みのプラグインの複製は、バージョンのディレクトリ名で識別します。
npm から入れた CLI のほうは `npm ls headsign` が答えます。

**このリポジトリがやっていないこと。** headsign 自身には、headsign プラグインを有効にする `.claude/settings.json` は置いていません。
これは意図的な設定です。
headsign の開発では、現在編集中の CLI を実行します。
これは `src/` からビルドされる `node plugin/dist/headsign.mjs` です。
実行対象は、このリポジトリ自身の `.headsign/` にあるワークフローです。
ここでリリース済みのプラグインを有効にすれば、次のバージョンを書いている最中に前のバージョンを運転席に座らせることになります。
その場合、想定外のことが起きるたびに、現在の変更とゲートを走らせている複製の二つが原因の候補になります。
これが「headsign を*使う*プロジェクト」と「headsign *である*プロジェクト」の違いです。
前者は、全員で同一の固定されたリリース版を使います。
ここまでの内容はすべて前者に当てはまります。
後者にとっては、そのファイルが無いことが設定です。

## プラグインなしで使う

プラグインは、headsign を Claude Code 向けにまとめた配布形態の一つです。
headsign の本体は CLI です。
ゲート判定、state、`PENDING`、ロック、ログはすべて CLI にあり、どのエージェントからでも、ターミナルから手作業でも使えます。
...
これらの方法で得たスキルはプラグインの外で動くため、同梱 CLI を見つけられません。
ただし、上記のとおりパッケージをインストールしておけば `npx headsign` にフォールバックします。
...
`SessionStart` は作業前に state を読んで案内し、`Stop` と `SubagentStop` はセッションと[委譲されたエージェント](#複数セッション)のバックストップになります。
run を委譲しない場合は、`SubagentStop` を省略できます。
`headsign claim` が使われていない限り、この hook は何もしません。

各行は、まずプロジェクト内のインストールを探し、次に `PATH` 上のものを探します。
最後に `node` を探します。
**どれか一つでも無ければ、何も出力せず exit 0 します**。
そのため、headsign を入れていないチェックアウトでも、アンインストールした後でも、同じ `settings.json` をそのまま置いておけます。
CLI が見つかっても実行できるとは限らないため、インタプリタを別に確かめます。
`node_modules/.bin/headsign` は `#!/usr/bin/env node` のスクリプトへの symlink です。
node をバージョンマネージャの shim で入れ、その shim が対話シェルでしか張られないマシンがあります。
そのマシンではファイルが在って実行可能でも、exit 127 になります。
プラグイン自身の hook も同じ理由で同じガードを持ちます([ADR-0005](adr/0005-distribution-and-toolchain.md) を参照)。
CLI が無いとき、backstop は黙って外れます。
どちらも同じ代償を受け入れています。
その場合、run はエージェント自身による `headsign next` の呼び出しに頼ります。
...
> クローンしたリポジトリには、コミットされた state ファイルや lock が含まれる場合があります。
> 自分で作成していない `.headsign/` は、ワークフローと同じく信頼できない入力として扱ってください。
> これはチームのリポジトリでも同じです。
> `.headsign/` への変更は同僚の PR に乗って届き、あなたのループで自動実行されるため、CI の設定への変更と同じ重みでレビューしてください。
...
ロール別の出来合いのワークフローは [example.headsign/](../example.headsign/) にあります。
対象は TDD 開発、バグ修正、ドキュメント執筆、リリースです。
自分で下書きしたワークフローと比較すると、出来合いのワークフローが同じフェーズをどう扱うかが分かります。

### 実行状態と、headsign がそれを探す場所

実行状態は `.headsign/state.json` に置かれます(自動で gitignore されます)。
状態がすべて外部にあるため、コンテキストの compaction を挟んでもループは生き延びます。
再開時は `headsign status` で現在のフェーズと成果物を確認し、未完了の作業を進めてから `next` を実行します。
必要な委譲は、権限を持つ運転役が開始します。
`RUNNING` は保存された run の状態で、プロセスの稼働状況ではありません。
未 claim の run でも、メインセッションが運転役になれます。
...
そこで見つかった run を、このセッションが最後に動かしていれば——あるいはまだ誰も動かしていなければ——hook は1行書いてターンを終えさせます。
引き留めはしませんが、無言でもありません。
`.headsign/log` には `by=CLAUDE_PROJECT_DIR` を伴う `unheld` 行が残ります。
`headsign status` の `last stop:` 行も同じ理由を名指しします。
Claude Code 自身の already-continuing フラグとは違う言い回しです。
これで、この問題のありふれた形は塞がります。
該当するのは、run 自身の git 境界の外へ `cd` したセッションです。
境界の外で立ち上がったセッションも該当します。
どちらも Claude Code が伝えてきたプロジェクトの内側にはまだいます。

最後に動かしたのが*別の*セッションである run は、ここでも、最初の探索が与えたはずの沈黙のまま通されます。
理由も同じです。
その行はターン終了の記録であり、run を一度も駆動していないセッションの記録を、run は必要としません。
これは [ADR-0027](adr/0027-recording-who-drove-a-run.md) が部外者に渡すのをやめた2本の `unheld` 行のうちの2本目です。
この行が無いと、この形は下にある完全な沈黙の形と同じに見えます。
`headsign status` の `last moved:` が両者を見分けます。
...
ルートより*下*にある run(monorepo のパッケージ)にも届きません。
*横*にある run(`git worktree add ../wt-feature` のようにチェックアウトの外へ足した linked worktree)にも届きません。
どちらも上ではないからです。
この無言は、セッション自身のプロジェクトにそもそも run が無い場合にも、どこに立っていようと同じように及びます。
そして、もともと無言ではなかった形が1つあり、この変更はそれをそのまま残します。
セッションが迷い込んだチェックアウトが*自分自身の* headsign ワークフローを実行している場合があります。
最初の探索はその run を見つけて、そちらについて催促します。
形式としては正しくても、対象が違う run です。
`headsign status` はこの2つを見分けてくれません。
見分けられるのは、セッションが実際にどこに立っていたかを知っていることだけです。

そもそもセッションがそこに立ちうる経路は見た目より狭く、それを知っておく価値があります。
自分の環境で起こりうるのかどうかが決まるからです。
hook のペイロードの `cwd` は、ターン中の `cd` に追随します(2026-08-01 実測)。
ただし Claude Code は、セッションの**許可された作業ディレクトリ**の外への `cd` を拒否し、拒否のメッセージでその一覧を示します。
...
それが run に届いていれば、`last stop:` がその事実を述べます。
届いていなければ、バックストップが無い状態と見分ける手掛かりは依然としてありません。
ただし run のディレクトリで `headsign status` を叩けば、1つ前の停止は見えます。
そのラン中にナッジが1度でも届いていれば hook が配線されていることの証明になります。
ワークフローのあるディレクトリかその配下で作業してください。
引き留められずにターンが終わり、その理由を説明できない場合があります。
その場合は、終了時にセッションがどこに立っていたかを疑ってください。
`last stop:` がどちらを名指ししているかも確かめてください。
この二つの理由は、[ADR-0006](adr/0006-stop-hook-backstop.md) の bounded walk-up の節と [ADR-0026](adr/0026-a-second-place-to-look.md) にあります。
この fallback は分岐を閉じるのではなく狭めます。
...
停止 hook は自分の目印 2 つ(`tmp/stop-note` と `tmp/claim`)を消費しますが、それ以外には触りません。
...
**ツリー単位ではなく、run 単位。** `limits.max_total_iterations` はこの run の周回を数えます。
`start` はその数を 0 に戻します。
そのため、同じツリーの上でもう一度 run を始めると、その二回目の run はまっさらな持ち分をもらい、フェーズの `max_attempts` もそこから数え直します。
**同じツリーの上でワークフローを何度も、順に走らせることは headsign が想定している形です。**
**同時に二本走らせることは想定していません。**
この二つは別の答えなので、分けておく価値があります。
同時のほうは決まっていて、強制されています。
`start` は `running` な状態を上書きせず、`next` か `abort` を先に済ませるよう促して exit 3 で終わります。
一方、終わった run の状態は次の `start` が何も言わずに上書きします。
通算のほうは何も縛っていません。
そしてそれは、抜けているのではなく決定です。
run をまたいで数える天井は無く、`start` を何回呼べるかにも上限がありません。
...
そして仕事が分割で届く形なら、走り直すか一つの run の中でループするかの分かれ目は一つの問いに落ちます。
**次の周は、前の周の試行回数と作業ファイルを見る必要があるか。**
...
あとで実行する `headsign start` は `state.json` をまるごと書き直し、`.headsign/tmp/` をまるごと消して、entry フェーズから始めます。
つまり run が `tmp/` に置いた mark と note は、次の run が始まるときに消えます。
abort された run から引き継ぐのはログだけで、ほかには何もありません。
だから abort する前に問うべきなのは、いつも*ここまで歩き直すのにどれだけかかるか*だけです。
その答えはワークフローの早いゲートを再び通すのにどれだけ費用がかかるかにあります([コマンドと出力の契約](#コマンドと出力の契約)に、それを安く保っておく理由があります)。

### 1 worktree に 1 run

**1 worktree に 1 run**、これが headsign の worktree サポートのすべてであり、構造上そうなっています。
linked worktree の `state.json`、lock、log は、その worktree 自身の `.headsign/` に置かれます。
headsign は共有の `.git` ディレクトリの下には何も書きません。
...
`split` フェーズの `description` は、仕事を独立した項目に割るようエージェントに指示します。
項目ごとに `git worktree add` し、その中で `headsign start` を実行するようにも指示します。
続く `gather` フェーズがその子 run を待ち、`integrate` フェーズが成果を取り込んで worktree を片付けます。

そこで headsign が何をしているかをよく読んでください。
見た目より少ないからです。
headsign は子 run を開始せず、worktree を作らず、待ちません。
headsign は子 run の存在も知りません。
フェーズの `description` がエージェントに頼むため、分岐が起きます。
これは「`/foo` スキルを使え」「レビュー用のサブエージェントに確認させろ」と書くのと同じ種類の指示です。
詳しくは[段取りとゲート](#段取りとゲート)を参照してください。
description は段取りを示します。
強制されるのはゲートだけです。
親の run は相変わらず一度に一つのフェーズしか動かしていません。
並列性はその一段下にあり、headsign からは見えません。
親の attempt と反復の天井は、親自身のゲート評価だけを数えます。
worktree を作ることも、片付けることも、最初から最後までエージェントの仕事です。
worktree を*管理しない*ことは、worktree の中で働かないことと同じではありません。

headsign が追加するのは、「全部そろったか」に答えるシェルコマンド一つです。
...
このため、待機は attempt を消費せず、失敗にもなりません。
そのうえでゲートが尋ねるのは「子が全員 `COMPLETE` か」で、そうでない子がいれば `on_fail: escalate` が run を人に渡します。
escalate や abort で終わった子は、すでに誰かの判断が要る状態だからです。
どちらも子の `state.json` を直接読まず、`headsign status` で子を読みます。
その 1 行目が `RUNNING` / `COMPLETE` / `ESCALATED` / `ABORTED` という文書化された契約だからです。

オーケストレータがモードとして提供する合流の方策には、全部そろう、どれか一つそろう、N 個の定足数があります。
headsign では、同じループのテストを差し替えてこれらを表現します。
`fan-out.yaml` はその三通りをコメントに書き出しています。
これらは headsign に足りていない設定ではありません。
この構造により、`needs:` と DAG 的な並列を拒否した判断を見直す必要がありません。
その判断は [ADR-0003](adr/0003-workflow-yaml-vocabulary.md) にあります。
DAG で表す対象は一段上で表せます。
対象を一段上に置くことで、headsign がオーケストレータへ拡大することを防いでいます。
...
ワークフローは、スキルやサブエージェントの仕事をゲート付きの順番に並べる*段取り*です。
ワークフローは仕事を*オーケストレーション*せず、使用するスキルも指定しません。
実際に効くのはゲートのほうです。
チェックの exit code だけが結果を確かめます。
あるスキルの使用を必須にしたいなら、その成果物を確かめるゲートを書きます(たとえば、そのスキルが生むファイルを `grep` します)。
レビューのような soft gate のフェーズでは、判定ファイルを `clear:` に挙げておくとよいでしょう。
`.headsign/tmp/verdict` などが判定ファイルです。
前回の判定が残っていると、今回の判定と取り違える場合があります。
headsign はフェーズ進入のたびに判定ファイルを削除します。
読み取り専用の reviewer が判定を報告したあと、Claude が毎回新しい判定ファイルを書きます。
**`clear:` が消すのはファイルで、ディレクトリは消しません。**
ディレクトリだった要素はそのまま残り、フェーズ進入時に `not cleared:` の行でそう告げられます。
ツリーが空になることを期待していたフェーズは、何周かしたあとではなく最初の進入でこの動作を知ります。
`headsign validate` も同じことを先に警告します。
対象は末尾が `/` で書かれた要素で、ファイルシステムに触らない検査で見分けられる唯一の書き方です。
ディレクトリを消す場合は、フェーズの作業として削除できます。
そこには削除の意図を書けます。
フェーズ進入のたびに黙って消える `clear:` 欄に書くことにもなりません。
判定そのものを作業エージェントの手から出したい場合は、チェック自体を審査者にできます。
たとえば `claude -p '… APPROVED か REJECTED だけを返せ' | grep -qx APPROVED` を使えます。
遷移の決定論を保ったまま、判定者だけが替わります。
トレードオフは [ADR-0007](adr/0007-verdict-authorship.md) にまとめてあります。

フェーズの意味は、そのゲートがシェルで確かめられる範囲までしかありません。
テストのゲートが証明するのは「何も壊れていない」ことです。
「機能が完成した」ことではありません。
...
レビューのフェーズは、エージェント自身のレビュー規律です。
これは、人間が PR をレビューすることの代わりではありません。

## 実行の流れ

ループを回すのは三者です。
エージェント(Claude)は作業を進めてループを駆動します。
**headsign** は現在のフェーズのゲートを実行し、トークンで答えます。
**チェック**は通常のシェルなので、判定は決定論的です。
周回のたびに Claude はトークンに従います。
`RETRY` なら、Claude は報告された失敗を直して再度尋ねます。
`ADVANCE` なら表示されたフェーズへ移り、失敗時ルーティング(`gate failed → routed to …`)なら作業が差し戻されます。
`COMPLETE` で run が終わります。
...
| コマンド | 役割 |
|---|---|
| `headsign start [name] [--workflow path] [--no-optimize]` | state を初期化し、entry フェーズの指示を表示する。改善は既定で有効 |
...
もう二つのコマンドは、道具そのものについて答えるため、この契約の外にあります。
`headsign version` は動いている複製のバージョンだけを表示します。
`--version` も同じものを表示します。
`headsign help` は使い方の文面を表示します。
`-h`、`--help`、引数なしの `headsign` も同じものを表示します。
どちらも常に exit 0 です。
...
そこにあるワークフローは、このプロジェクトのパスと道具を参照するため、サンプルと分けています。

無引数の `headsign validate`(name も `--workflow` も指定しない場合)は、現在の run が実際に使っているワークフローを検証します。
`.headsign/state.json` が存在すれば、status を問わず、その run 自身の `workflow_path` を検証します。
この条件では、固定のデフォルトファイルではなく、その run 自身の `workflow_path` を検証します。
そのため `headsign start <name>` で開始した run を検証するときは、名前を繰り返す必要がありません。
正しい `.headsign/<name>.yaml` が検証されます。
...
これにより、書きかけのフェーズや一時的にコメントアウトした辺が、進行中の run を止めません。
`start` も、ファイルを書いた本人がその場にいる間に警告を一度だけ表示します。
毎周回実行する `next` は警告を表示しません。

もう一つの警告は、到達ではなく**停止**についてのものです。
あるフェーズの集まりが **pass 辺だけ**で循環できる場合があります。
そこで `limits.max_total_iterations` が宣言されていなければ、その run を止めるものは何もありません。
`max_attempts` は「そのフェーズが最後に pass してからの失敗回数」なので、pass で回る閉路は毎周それを消してしまうからです。
[example.headsign/sweep.yaml](../example.headsign/sweep.yaml) のような掃引が、その形です。
そのため、このワークフローは上限を宣言しています。
**失敗**辺を通って閉じる閉路は警告しません。
失敗するフェーズの `max_attempts` が、実際に上限として効くからです。
詳しくは [ADR-0022](adr/0022-validate-checks-that-a-run-can-end.md) を参照してください。
...
メッセージはその階層が受け付けるキーを列挙し、綴りを推測しません。
同じ考え方は `version:` にも及び、値はちょうど `0.1` でなければなりません。
スキーマは 1.0 より前で、まだ変わり続けています。
そのため、headsign は停止します。
ファイルを書いた本人が、フィールドを現在のスキーマと照合します。
たまたま今も通るフィールドだけで読み込まれることはありません。
詳しくは [ADR-0015](adr/0015-strict-schema-and-version-0-1.md) を参照してください。
...
**`next` はゲートの実行中、進捗を stderr に書き込みます。**
最初の検査が始まる前に、1 行目がそのゲートの持つ検査の本数を名指します。
終わった検査 1 本ごとに、もう 1 行が続きます。
その行は、検査がどう終わったかも示します。
...
ゲートは最初の失敗で止まります。
そのため `passed` 以外を言う行は多くても 1 本で、それが最後の行です。
語は `passed`、`failed`、`timed out` の 3 つです。
`timeout:` で打ち切られた検査は timed out と表示されるため、同じ時間がかかった通常の失敗と読み違えることはありません。

**行が示す経過時間が、その検査の上限の半分に達したときは、上限も名指します。**
どの検査にも上限があります。
書いた側の `timeout:` があればそれ、書かなかったときは headsign の既定値である 120 秒です。
半分より手前では、行は上と全く同じ形のままです。
半分に達するか超えると、2 つ目の数が加わります。

```
--- check 5/12 passed: acceptance matrix (60.4s of 120s) ---
```

半分の地点で何かが失敗するわけではなく、それに縛られるものも何もありません。
上限はワークフロー自身の `timeout:` であり、この地点は経過時間をその上限と読み合わせる価値があるかどうかを決めるだけです。
比べるのは行が印字する数そのもので、10 分の 1 秒に丸めた値です。
そのため、この規則は行を見るだけで確かめられます。
同時に、`timeout:` がごく小さい検査は経過時間が `0s` に丸まるので、実際には上限のどれだけを使っていても半分の地点には届きません。
`timed out` の行は必ず 2 つ目の数を持ちます。
これを決めるのは経過時間の丸めではなく、事実そのものです。
打ち切られた検査は、測るまでもなくその上限に達しています。
経過時間は小数点以下 1 桁に丸めた値です。
0.1 秒に満たない `timeout:` では、その丸めた経過時間が 0 になり、半分の地点を下回ることがあります。

exit code を一度も返さなかった検査には行が付きません。
headsign はその周を拒み、試行を消費しません。
拒否の行が、その検査とコマンドをすでに名指しているからです。
ゲートが届かなかった検査にも行は付きません。
手前の検査が落ちて、ループがそこで止まったからです。
これらの行を有効にするフラグはありません。
必要とする読み手は、ゲートが遅いとは予期していなかった読み手だからです([ADR-0032](adr/0032-the-gate-says-how-far-it-got.md))。

**ゲートの途中で `next` が終了させられても、何も消費されていません。**
`step()` は、ゲートが答えを返したあとにだけ試行を加えます。
そのため、途中で終了させられた周は、試行を 1 つも書いておらず、フェーズも動かしていません。
ロックも安全です。
`acquireLock` は、保持者のプロセスが既に無いロックを置き換えます。
そのため、途中で終了させられた `next` のあとに、片付けるものは何もありません。

**`RETRY` を返すゲートの失敗、および `on_fail` で先へ回される失敗は、どの検査に届かなかったかを言います。**
検査は順に走り、ゲートは最初の失敗で止まるので、その後ろの検査は 1 本も走りません。
そして**落ちた周は、そのゲートが最も薄く検査した周**です。
ブロックはそれを名指します —— `--- 2 of 3 checks ran; 1 not run: deferrals tracked ---`。
同じ数は `.headsign/log` の retry 行と routed-fail 行にも `ran=2/3` として載るので、run が終わったあとでも答えられます。
落ちたのが最後の検査だったときは、どちらも出ません。
**`max_attempts` を使い切る失敗は例外です。**
それは run を終わらせる `ESCALATE` で、その行が持つのは理由だけです。
そのため `max_attempts: 1` のフェーズでは、このブロックが stdout に出ることは 1 度もありません。
本数と到達した位置は、ゲートが走っている間に stderr へ届いています。
これは他のどの周でも同じです。
ワークフローファイルを見る側になお残っているのは、走らなかった検査の**名前**です。
進捗の行は、走らなかった検査を 1 本も名指さないからです。
これがいちばん効くのは、落ちた周でループが終わった場合です。
失敗の後ろにある検査は、その走行の中で 1 度も走っていないことがあり、**「通った」と「走っていない」は、そうでなければ同じ沈黙**になります。
...
代わりに、実際に残っている二つの読みを書きます。
何かを変えたのにこの検査がそれを読んでいないのか、それとも何も変わっていないので、残りの試行を使う前に**このゲートがそもそも通りうるのか**を確かめるべきなのか。
...
**通った検査の出力は破棄されます。**
出力を保持するのは失敗の枝だけで、だから上の表でも出力が続くのは `RETRY` の行だけです。
exit 0 で終わった検査の stdout と stderr は落ち、どこにも記録されません。
これが効くのは 1 つの形です —— **検査するものが無かったから通った検査**。
「35 件すべて正しかった」と「対象が 1 件も無かった」は、同じ exit code で、記録の上でも同じ 1 行になります。
件数を stdout に書いても助けになりません。
落ちるのはまさにその出力だからです。
空振りしうる検査は、**何件を検査したかを、後段の検査が読む場所に書いてください**。
この run だけが使うなら `.headsign/tmp/` の下に書きます。
走行の成果物の一部なら追跡されるファイルに書きます。
そうすると件数は語られるのではなく**ゲートに掛かる**ようになり、「何も検査しなかった」がゲートで落とせる事実になります。

exit 3 は設定または使用方法のエラーです。
check や `ready:` プローブが**そもそも実行できなかった**場合も含みます。
該当するのは、コマンドが起動しなかった場合と、headsign が終わる前に止めざるを得なかった場合です。
それはゲートの失敗ではありません。
...
走った上で**タイムアウトした** check は別物で、従来どおり通常の失敗です。
実際に走っており、超えた制限時間はあなた自身が書いたものだからです。
終了済みの run に対する `next` は冪等です。
動いている run に対しては、`next` は様子見ではなく判定です。
そのフェーズのゲートを実行し、失敗すれば試行を 1 つ消費します。
`ready:` プローブがまだ通っていないフェーズは、ゲートの手前で `PENDING` を返します。
この場合は何も消費しません。
...
チェックは CI で見慣れた `- name:` / `run:` / `timeout:` のステップです。
`/bin/sh -c` で実行され、最初の失敗でゲートは打ち切られます。
headsign が実行するコマンドはすべて headsign 自身の環境を引き継ぎます。
環境変数が必要なチェックは、プロンプトで書くのと同じように、自分の `run:` の文字列の中で設定してください(`run: "FOO=bar npm test"`)。
これとは別に、headsign 自身が設定する変数が 1 つ加わります(後述の「環境変数」の `HEADSIGN_WORKFLOW_FILE`)。
...
そこでの `/bin/sh` は bash 3.2 です。
変数の直後に非ASCII文字が続く `run:` の文字列を書くと問題が起きます。
該当するのは `"$now foo"` ではなく `"$now→"` のような形です。
そのシェルでは変数が空に展開され、直後の文字の先頭バイトまで一緒に飲み込まれます。
...
チェックは headsign 自身の環境を引き継ぐので、実際に効くのはあなたが headsign を動かしている `LANG` です。
...
残る二つは run を `running` のまま残し、答えを待ちます。
run の足元でグラフが変わった場合([run が歩いているグラフ](#run-が歩いているグラフ))と、下記の天井です。
...
暴走防止はそのままです。
`headsign status` の答えも `RUNNING` のままです。
詳しくは [ADR-0017](adr/0017-three-budgets-and-the-recoverable-ceiling.md) を参照してください。
run が本当にまだ終わっていない以上、停止境界の hook は駆動役を `headsign next` へ押し戻し続けます。
天井を報告して席を立つエージェントは、その前に一時停止のメモを書いてください([バックストップ](#バックストップ)を参照)。

この三つの予算には共通点があります。
どれも headsign が `next` 一回の中で、誰にも尋ねずに自分で数えられるものだ、ということです。
トークンと料金は数えられません。
headsign はモデルを起動していないので、直前のターンが何を使ったのかを見る手段がそもそもありません。
これは欠落ではなく層の境界で、それらの数字が存在する場所はあなたのハーネスの側です。
仕込めばチェックが読みにいくことはできますが、代償としてワークフローが特定のベンダーのインターフェースに結合します。
経過時間だけは headsign にも数えられます(`.headsign/log` は遷移のたびに時刻を打っています)。
それでも数えないのは判断です。
遅い run は間違った run ではありませんし、ループが支払う単位は周回だからです。
...
- ルートが解決されるのはゲートが**通過したあと**だけで、失敗経路では一切解決されません。
  ルーターのフェーズも、自分のゲートが落ちたときはただの失敗したフェーズです。
- `when:` は上から順に実行され、**最初に exit 0 になったもの**が勝ちます。
  どれも一致しなければ、最後の要素の `to:` が使われます。
- `when:` は任意で `timeout:`(秒、既定は 120)を取れ、headsign 自身の環境で実行されます。
  チェックとまったく同じ扱いです。
- `to:` にはフェーズ名か `$end` を書きます。
- `validate` は、最後の要素に `when:` がある並び(既定の行き先が無くなります)と、最後より前の要素に `when:` が無い並び(その後ろがすべて到達不能になります)を拒否します。
- `when:` が**そもそも実行できなかった**とき、つまり spawn に失敗したときやタイムアウトしたときは、headsign は既定の行き先へ流れ落ちるのではなく、exit 3 で停止してどこへも遷移しません。
  非 0 の exit は「これではない」という答えですが、一度も走らなかったコマンドは答えではなく、ここで決めているのは run が次にどこへ行くかそのものだからです。
...
レビューフェーズのゲートは、ループより遅い処理に依存することがよくあります。
...
さらに、そのフェーズの判定ファイルを `clear:`(前に推奨した設定)に挙げると、早すぎる呼び出しによる再入場が、少し遅れて届いた判定を削除する場合があります。
フェーズには `ready:` プローブを持たせられます。
たとえば `test -f .headsign/tmp/verdict` です。
この場合、早すぎた `next` は `PENDING` を返します。
試行回数は消費されず、`clear:` も走らず、判定ファイルは、実際にそれを見つける `next` のためにそのまま残ります。
そうしなければ、本物のレビューが静かに失われます。

### バックストップ

スキルは指示であって、強制ではありません。
そこで 2 つの停止境界 hook が `.headsign/state.json` を読みます。
run が `running` の間は、その run の**駆動者**のターン終了をブロックします。
そして `headsign next` に差し戻します。
駆動者のものでないターンは、そのまま通ります。
`claim` marker が構えられている間は例外です。
停止した委譲エージェントのうち、自分を名乗れた最初の一つが新しい駆動者として着席します。
詳しくは[複数セッション](#複数セッション)を参照してください。
2 つの hook は、escalated、aborted、completed の run も通します。
どれも正しい終わり方だからです。
...
駆動者への催促を逃す影響が、余分な催促 1 回の影響より大きいからです([複数セッション](#複数セッション)に両方が書いてあります)。
`SubagentStop` は通します。
近くで止まる委譲エージェントの多くは、その run に役目を持たないレビュアーや作業者です。
そのようなエージェントを引き止める影響が、催促を 1 回逃す影響より大きいからです。

セッションの記録が不明な場合、Stop の催促は、受信者が駆動者かどうかを headsign が判別できないと伝えます。
`next` の実行は、run を開始したセッションか、継続を依頼されたセッションに限って案内します。
それ以外のセッションには、`next` と `abort` を実行せずターンを終えるよう、一時停止と中止の案内より先に伝えます。
末尾では、セッションを起動する環境に設定する `HEADSIGN_OBSERVER` を案内します。
セッションの記録が一致する場合と、すべての `SubagentStop` の催促は、従来の文面を保ちます。
...
**意図的に中断するには**、`.headsign/tmp/stop-note` に**何を待っているか**を 1 行書いてから、もう一度停止してください。
hook は即座に通り、差し戻しは不要で、`.headsign/log` に `paused` 行が残るので中断の記録が残ります。

**名指せるかどうかが検査です。**
「なぜ止まるか」は自分の状態を説明すれば答えられてしまいます。
自分がどこに居るかだけを書き、何を待っているかを名指していない note は、行き先が在るのに止まった run です。
「何を待っているか」にはその答え方がありません。
委譲先、人、ビルドのいずれかを名指すか、名指すものが無いと気づいて進むかのどちらかです。
記録も読めるようになります。
log は note の 1 行目を持つため、待ち先を名指した中断と、名指していない中断をあとから区別できます。
note は読まれた瞬間に消費され(削除され)、作業ツリーは中断前とまったく同じ状態(正味無変化)に戻ります。
一時停止それ自体は run に何の負担もかけません。
そのフェーズの成果物は、作業が残したまま置かれます。
ログには note の 1 行目を 120 文字までに切ったものが残ります。
それが note 全文と一致しない場合は、末尾に `…` が付きます。
途中で終わった note を完結した note と読み違えることはありません。
note 1 つが覆うのはターン終了 1 回ぶんなので、待ちが何度かのやり取りにまたがる場合は、その都度書き直してください。
再開時はまず `headsign status` を読み、フェーズの作業を終えてから `headsign next` でゲートを判定します。
...
意図的に run を終わらせると run は失われますが、履歴は残ります。
...
`next` のたびにゲートがもう一度走るため、同じ性質は毎周回にも影響します。
早いフェーズのゲートが遅い、あるいは冪等でないワークフローは、自分自身の再実行コストを自分で高くしているのです。
それはワークフロー作者が背負うべきコストであって、headsign が肩代わりできるものではありません。
...
この上限は、行き詰まったエージェントや黙って離脱したエージェントへの保険です。
通常の中断には前述の note を使います。
...
この判定はその 2 つから読み取ってください。
`.headsign/state.json` の上限カウンタは、実際の `headsign next` ごとに 0 に戻るため、駆動されている run ではほとんど何も教えてくれません。
...
そのため、催促はターン終了 1 回ごとではなく、おおよそやり取り 1 回につき 1 度届きます。
窓の幅はターン 1 つぶんで、そのターンが終わると閉じます。
これは上の nudge 上限とは別の仕組みで、外から見える結果だけが同じです。
現在は、両者が残す記録で区別できます。
上限を使い切ったほうには `stalled` 行があります。
覆されたターン終了は `.headsign/log` に `unheld` 行を残します。
この行は告げられたフィールド名を `by=stop_hook_active` として載せます。
`headsign status` の `last stop:` 行も残ります。
...
headsign が加えるのは、その変化が黙って起きないようにすることだけです。

`start` の時点で、run は自分が歩いている**規則**の fingerprint を記録します。
対象は、現在地から到達可能なすべてのフェーズと `limits` です。
fingerprint は規則を対象とし、指示を対象としません。
フェーズの `description` は意図的に対象外なので、エージェントへの指示を書き換えても何も起きません。
`gate` / `ready` / `clear` / `on_pass` / `on_fail` / `max_attempts` は対象です。
(`clear:` が規則に入るのは、それを外すことが、前のパスの `APPROVED` をレビューゲートに通す手口そのものだからです。)
コメントや整形も不可視です。
fingerprint はバイト列ではなくパース後の構造から取ります。
...
`checks/coverage.sh` を書き換えて判定を逆にすると、fingerprint はそのままでもゲートの判定は変わります。
次の周回は新しい規則で判定し、何も報告しません。
`run:` は任意のシェル文字列なので、headsign はコマンドが読む対象を特定できません。
これは頑張れば塞げる穴ではありません。
...
ある周回でその規則が変わっていたとき、headsign はその変更を報告します。
run を `running` のまま残す `ESCALATE` で、試行も反復も消費せず、動いたフェーズを名指しします。
そこから先の道は二つです。

- **ファイルを元に戻す。**
  次の `next` は再び fingerprint と一致し、何も言わず、何も要求しません。
  戻すことは無料です。
- **`headsign next --accept-graph-change` を実行する。**
  これが受け入れで、run はそのまま進みます。
  受け入れた変更は計上され、`COMPLETE` がその回数を言います。
  `.headsign/log` は gitignore されていて PR には届きませんが、最後の答えは報告を受ける人が読むからです。

**`headsign status` は、記録だけでなく目の前のファイルについても答えます。**
差分が `state.json` に届くのは、周回がそれを報告したときだけです。
編集してから次の `headsign next` まで、記録はその編集について何も持っていません。
`status` はディスク上の規則を読んだその場でハッシュし、pin と突き合わせることでその窓を塞ぎます。
ファイルが記録の知らないことを言う場合、`graph:` の行は次の二つのどちらかです。
...
そのため、読む側は一つの規則で判定できます。
`graph: the file no longer matches …` が出ているか、`graph: changed since this run accepted it …` の下に `graph: the file matches … again` が無ければ、ファイルは pin と食い違っています。
これを `--- phase: ---` ブロックと合わせて読んでください。
ワークフローが読めないときそのブロックは出ません。
読めるファイルが無いことは比較対象が無いことなので、`status` は推測せず何も言いません。
この比較はゲートを走らせず、プロセスを起こさず、何も書かず、ロックも取りません。
`status` は見るための道具のままです([ADR-0029](adr/0029-status-answers-for-the-file.md))。

**素の `next` は、何回叩いても受け入れません。**
同じ変更をもう一度報告し、何も計上せず、試行も反復も消費しません。
これは意図的です。
受け入れと再試行は以前**同じコマンド**でした。
途中で出力を読まずに `next` を 2 回以上発行するものがあります。
たとえば、バッチ、ループ、非ゼロで再試行するラッパーです。
それらは、**誰も報告を見ないまま規則の変更を受け入れられました。**
このフラグは「人が実行した」ことの主張ではありません(headsign にそれは分かりません)。
**2 つの行為を別の入力にするためのもの**です。
習慣で付けておくこともできません。
報告されている変更が無い状態では、素の `next` として振る舞うのではなく exit 3 で断ります。

意図的に黙っているものが二つあります。
run が依存しないため、その run がもう到達できないフェーズへの変更は報告されません。
そして `limits` だけの変更は報告なしで受け入れられます。
天井に当たってから上げるまでが二度ではなく一度で済むようにするためで、計上はされるので「余裕を与えられた run」であることは最後に分かります。

これは錠前ではなく手すりです。
ワークフローを編集できるものは `.headsign/state.json` も編集できますし、headsign はそれについて何も言いません。
やっているのは、**緩められたゲートと、ドキュメントが勧める編集とを切り分ける**ことです。
今まではそれが同じ行為でした([ADR-0023](adr/0023-pinning-the-graph-a-run-is-walking-under.md))。
...
設計を導いた実戦フィードバックは [ADR-0008](adr/0008-multi-session-ownership.md) を参照してください。
そこから絞り込んだ現在の形は [ADR-0013](adr/0013-claim-only-driver-identity.md) を参照してください。
...
この語を使い回すと、そのフィールドが持たない意味を語ることになります。
詳しくは [ADR-0027](adr/0027-recording-who-drove-a-run.md) を参照してください。
claim されるまで、headsign はいまも誰が駆動しているかを知りません。
ただしいまは、その run を動かしたことのあるセッションと、一度も動かしていないセッションを見分けられます。
`last_drive` にセッションが記録されていない run では、headsign は従来の催促の判定を保ち、駆動者が不明であることを伝えます。
対象には、このリリース以前に始まった run がすべて含まれます。
Claude Code の外から駆動している run も対象です。
そこにはセッションを名指すものが無いため、`start` も `next` も記録するものを持ちません。
人が手で編集した state も対象です。
そのディレクトリで止まったセッションはすべて催促されます。
`start` か `next` がセッションのスタンプを記録したあとは、そのセッションのターン終了だけが引き留められます。
それ以外のセッションの停止はすべて無言で通り、委譲されたエージェントはどちらの場合も一つも引き留められません。
run が claim されたあとは、引き留められるのはそのエージェントのターン終了だけになります。
どのセッションの停止も通ります——claim された run では `last_drive` はそもそも読まれません。

この二つのふるまいが、run の取りうる二つの形をそのまま覆います。
自分の run を自分で駆動しているセッションに claim は要りません。
`start` は run の開始時に、そのセッションを「動かした者」として刻印します。
最初のターンから `last_drive` はそのセッションを名指しします。
ほかの誰も claim していない間は、そのセッションが催促され続けます。
それが、そのセッションが望むバックストップです。
...
この仕組みは、以前は意図的に見分けなかった二つのセッションを見分けます。
一方はその run を動かしたことのあるセッションです。
もう一方は同じディレクトリに立っているだけの二つ目のセッションです。
run はいまもそれが置かれたディレクトリのものです(1 worktree に 1 run)。
ただし最初のセッションが `start` か `next` を実行すると、二つ目のセッションはもう催促されません。
その停止は無言で通り、上限を1つも消費せず、どこにも1行も書きません。
claim された run で駆動者でないセッションの停止が通る場合と同じです。
これには人が開いたのではないセッションも含まれます。
プログラムが Claude Code を子プロセスとして起こす場合があります。
その子は呼び出し元のいた場所に立つセッションになります。
別のディレクトリを与えていなければ、そこは run のディレクトリです。
その run を他の誰かが動かしていれば、その子プロセスの停止も同じように通ります。
これはまさに [ADR-0027](adr/0027-recording-who-drove-a-run.md) が直した不具合です。
修正前の子プロセスには、催促が自分宛てでないと知る手立てがありませんでした。
そのため、子プロセスは催促に応えようとしました。
呼び出し元には、起動した目的の出力の代わりに headsign についての散文が返りました。
その途中で、run の本当の駆動者が必要とする上限も1つ消費しました。

一つだけ穴が残ります。
しかもそれは逆側の当事者に降りかかります。
誰かが始めた run を引き継いだセッションを**ハンドオーバー**と呼びます。
`SessionStart` hook は作業開始前に、実行中のワークフロー、フェーズ、直前の pause note を表示します。
`headsign status` と `headsign next` も案内しますが、run は変更しません。
ハンドオーバーも、自分自身が最初に止まった瞬間には催促されません。
自分の最初の `next` がそのセッションを刻印するまで、この状態が続きます。
これは部外者ではなく、その run の次の駆動者です。
それでもその間はバックストップを失います。
`Stop` には、不一致の停止と、その run に一度も触れていないセッションの停止が同じに見えるからです。
どちらを催促しても部外者まで催促するため、スタンプは何も買いません。
この代償は現在、バックストップだけに残ります。
run が既にあるリポジトリを開いたセッションは、これまで催促されることでそれに気づいていました。
それは近くにいる全員を催促する副作用でした。
読み取り専用の `SessionStart` hook が、催促を使わずに発見の合図を出します。
`headsign status` は読み取り専用の詳細表示を提供します。
`HEADSIGN_OBSERVER`(後述)は、自分が観察するだけだと分かっているセッションが、上の修正に頼らず明示的に降りるための手段です。
ここで headsign が提供する手動の制御はいまもこれだけです。
その位置づけは「そうしたセッションが催促を免れる唯一の方法」から変わりました。
現在は「意図的で、より確実な opt-out」です。
多くの部外者は、放っておかれるためにこれを必要としません。
駆動していないセッションは、`next` ではなく `headsign status` を使ってください。
teammate と、その run を任されていないサブエージェントが該当します。
一度も `headsign start` を実行していないセッションも該当します。
この規則は以前にも増して当てはまります。
他の誰かがすでに動かした run は、近くで止まった者を催促しません。
プラグインはセッション開始時に run を知らせ、`headsign status` が詳細を表示します。
...
run が `RUNNING` のあいだは、最後に 1 行空けて現フェーズの指示が付きます。
`start` や `next` が出すのと同じ `--- phase: <name> ---` のブロックです。
このブロックをそのまま渡します。
記憶から書き直したものではありません。
`next` はゲートを走らせて試行を 1 回消費する場合があるため、判定せずにフェーズの要求を読み直せるのはここだけです。
この機能は、作業を委譲するときに特に役立ちます。
headsign を一度も呼ばないエージェントには、ゲートの要求が見える経路がありません。
そのため、このブロック自体をエージェントに渡します。
...
`next` のトークンと同じく大文字表記ですが、これは*報告*です。
...
パッチリリースも含みます。
負っているのは変更履歴に書くことだけです。
**この出力を読む道具を書くなら、試したバージョンを固定してください。**
**包括的なパターンではなく、文字列で厳密に一致させてください。**
**一致しなくなったら、その場で落としてください。**
見慣れない出力を推測で読む読み手は、自信のある間違った答えを出します。
それは答えが無いことより悪い結果です。
詳しくは [ADR-0030](adr/0030-the-token-line-is-the-contract-and-nothing-else-is.md) を参照してください。
...
この行は、引き継ぎの成立を確認できるようにします。
`claim` は 2 拍の手続きで、静かに失敗することがあります。
そのため、確認メッセージのあとに `headsign status` を 1 回実行します。
委譲側のセッション、ユーザー、通りすがりの観察者の誰でも、run の駆動者が実際に替わったかを確かめられます。
これは誰でも確かめられる方法になります。

`RUNNING` の run には、これに加えて 3 行が出ることがあります。
最後の例にある 3 行は、run の現在地ではなく、ターンの*終わり方*か run の*動いた時刻*を示します。
どの行も条件付きです。
`last stop:` は、この run のものだと帰属できる停止を headsign が 1 回でも処理したあとに現れます。
そのターン終了をどう扱ったかを、次の 5 通りのいずれかで述べます。

- `held, and pointed back to headsign next` — 通常の催促です。
  同じ停止が `.headsign/log` の `held` 行です。
...
- `not held — Claude Code had already resumed the turn (stop_hook_active)` — そのターン終了で headsign は覆されました。
  同じ停止が `.headsign/log` の `unheld` 行です。
- `not held — the session was not standing in the run's tree (CLAUDE_PROJECT_DIR)` — セッション自身のディレクトリからの探索は run を見つけられず、代わりに `CLAUDE_PROJECT_DIR` からの探索が見つけました。
  hook は書いて、引き留めずに戻ります。
  同じ停止は `.headsign/log` でも `unheld` 行ですが、detail は `by=stop_hook_active` ではなく `by=CLAUDE_PROJECT_DIR` です — disposition は同じで、原因になった upstream の事実だけが違います。
  この行が書かれるのは、通常経路なら催促されていたはずの当事者に限られます。
  この run の `last_drive` が別のセッションを名指していれば、通常経路と同じ判定によってこちらでも行が差し止められます([ADR-0027](adr/0027-recording-who-drove-a-run.md) §9)——その停止は代わりに下の表の5行目に落ち、`.headsign/log` には何も残りません。

いずれのあとにも ` — at <タイムスタンプ>` が続き、記録されたとおりに、オフセットまで含めてそのまま表示されます。
この文言は、渡されたフィールドを headsign がどう扱ったかを示します。
そのフィールドに対する Claude Code 自身の説明は対象外です。
disposition はタイムスタンプと一緒に読んでください。
headsign が帰属できないターン終了もあります。
無関係なエージェントのものや、opt-out した環境からのものです。
この場合は行を空にせず、以前の停止についての記述を残します。
つまり古い disposition が残っていることはあり、それを見抜く手掛かりがタイムスタンプです。

`last moved:` は、その run の `last_drive` にセッションが記録されているときだけ現れます。
このリリース以前に始まった run では出ません。
Claude Code の外から駆動している run や、人が手で state を編集した run でも出ません。
...
`last stop:` と `last moved:` は別々の問いに答え、それぞれ独立に古くなる場合があります。
前者は、あるターン終了が最後に**帰属できた**時刻で、後者は run が最後に**動いた**時刻です。
両方をあわせて読むと、二つの状況を区別できます。
`last stop:` が新しく `last moved:` が古ければ、誰かはいますが run は進んでいません。
両方が古ければ誰もいません。
どちらの読みも、*あなた*がそこに名指されたセッションかどうかは言いません。
識別子そのものは決して印字されません。
理由は上の `driver:` と同じです。
`status` は環境にしか尋ねられず、「それはあなたです」と正直に答えられません。

この行は消えることもあります。
`start` と `next` が記録するのは、セッションを名指せるときのそのセッションです。
セッションを名指すもののない環境から、どちらかを一度実行する場合があります。
たとえば、人が端末から run を駆動する場合です。
記録するものが無いため、それまで刻まれていたスタンプは消えます。
run は、そのディレクトリで止まった者を誰でも催促する状態に戻ります。
これは意図してそちら側に倒しています: headsign はもう保証できなくなったスタンプを持ち続けません。
そしてスタンプの無い run は、誰も催促しないのではなく全員を催促する run です。

`entered:` は、この run が今立っている場所に**最後に到着した**時刻です。
これは前述の 2 つとは別の、3 つ目のタイムスタンプです。

```
entered: 2026-08-23T07:49:15+09:00 — when this run last entered the phase above
```

動くのはフェーズの `clear:` が走る瞬間ちょうどで、これはスキーマが既に引いている境界です。
`on_fail: retry` はフェーズに留まり `clear:` を走らせないので、エージェントが同じ失敗に取り組んでいるあいだ刻印は動きません。
`on_fail: <同じフェーズ>` は出て入り直すので動きます。
この点が前述の `last moved:` と異なります。
`last moved:` は答えにかかわらず、`start` と `next` のたびに刻まれます。
2 つを並べて読んでください。
`entered:` はこのフェーズが何分続いているかを言い、`last moved:` は誰かが最後に run を触ったのがいつかを言います。
新しい `last moved:` の下に古い `entered:` があればリトライを重ねているフェーズで、両方古ければ誰も駆動していない run です。
経過時間は引き算で出してください。
headsign は記録したとおりのタイムスタンプを出すだけで、自分では時間を計算しません。
詳しくは [ADR-0031](adr/0031-when-the-run-entered-the-phase.md) を参照してください。
このフィールドができる前に始まった run には刻印が無く、行も出ません。
...
停止をまだ一度も処理しておらず、`last_drive` の記録も無い run があります。
スイッチの無い環境で読むと、この 3 行のどれも存在しなかったときと同じ `status` の出力が出ます。
どの行も判定ではありません。
...
どちらもワークフロー名とフェーズ名を挙げ、`headsign next` の実行を促し、一時停止と中止の案内を含みます。
通常の催促は、末尾に observer の案内も付けます。
`SubagentStop` から届く `headsign workflow '…' is still running` で始まる催促は、あなたが既に駆動者であることの確認です。
駆動者が不明だと伝える Stop の通知は、この確認にはなりません。
run を開始しておらず、継続も依頼されていなければ、`next` と `abort` を実行せずターンを終えます。
...
他の行はどれも、ログか `status` のどちらかに何かを残します。
ただし、この run を最後に動かしたセッション以外からの停止は、どこにも一行も書かれません——意図的なふるまいです。
その run を一度も駆動していない部外者の停止を、run の記録として残さないためです。
詳しくは [ADR-0027](adr/0027-recording-who-drove-a-run.md) を参照してください。
...
**detail が `by=CLAUDE_PROJECT_DIR` の行が言うのは別のことです。**
**どの stop hook もこのターンを引き留めていません。**
...
`Stop` が停止を除外するのは、委譲されたエージェントがその run を握っているときと、run にスタンプされた `last_drive` が別のセッションを名指すときです。
...
いま引き留められた場合、run は多くの場合、あなたを「動かした者」として記録しています。
これは所有していることより狭い意味です。
しかもスタンプの無い run は、いまも誰でも引き留めます。
したがって、引き留めは確実な証明ではありません。
...
**委譲されたエージェント**、つまり Claude Code の agent-teams 機能における teammate や、サブエージェントは、headsign が記録できる唯一の駆動者です。
ほかの誰とも見分けられる唯一の相手です。
この種のエージェントは、自分を spawn したセッションのプロセスを共有しています。
PID も環境も同一です。
Bash tool の手が届く範囲のどこにも、自分固有の識別子を持ちません。
...
ログも教えてくれません。
`unheld` 行は、headsign が run に帰属させられた停止だけに書かれます。
claim されていない run では、そのエージェントのターン終了について何も残りません。
...
駆動者にはバックストップも適用されます。
いったん着席すれば、run が `running` の間、そのエージェント自身のターン終了は `headsign next` に差し戻されます。
stop-note による一時停止も `headsign abort` による終了も、セッションの場合とまったく同じように使えます。
...
**席は、そこに座っていたエージェントより長く残ります。**
**引き継ぎのうち、誰にも知らされていないのはこの部分です。**
ディスク上の状態があるおかげで、駆動していたエージェント(とそれを抱えるセッション)が消えても run は生き延びます。
API の障害、コンテキストの枯渇、人が作業を止めた場合、どれでも同じです。
...
headsign にはそれを見分ける手段がありません。
agent id はそれを渡されたプロセスの内側でしか意味を持たないので、生きているかどうかはこの道具が問える種類の問いではないからです。
この制約は引き継いだ側に影響します。
駆動者が記録されている間、`Stop` はどのセッションのターン終了も通します。
`SubagentStop` は id が一致するエージェントだけを引き留めます。
`SubagentStop` が引き留めるのは id が一致するエージェントか、構えられたマーカーのもとで最初に名乗ったエージェントだけです。
つまり**席が入れ替わるまで、実際に駆動している者のターンは誰にも保持されません。**
run は続きますが、実際の駆動者にはバックストップが作用しません。
...
セッションから `headsign claim` を打つのは無害な no-op でもありません。
あのコマンドが確かめるのは run の存在だけです。
そのため、構えられたマーカーはそのまま残ります。
そのディレクトリの下でターンを終えた次の委譲エージェントが、それを消費して着席します。
そのエージェントは、run に何の役割も持たない reviewer かもしれません。
そのセッションは run を問題なく駆動できます。
`next` は誰の許可も求めません。
...
反対に、headsign は `HEADSIGN_WORKFLOW_FILE` という変数を設定します。
設定先は、ゲートのチェック、フェーズの `ready:` プローブ、`on_pass` route の `when:` の実行環境です。
値は、この run のために headsign が記録したワークフローのパスです。
`state.json` の `workflow_path` と同じ文字列で、正規化も解決もされていません。
run を名前から起動していれば相対パス、絶対パスの `--workflow` で起動していれば絶対パスです。
これが在る理由は、ワークフローが自分自身を読めるようにするためです。
配布されたワークフローの空欄を誰かが埋めたかどうかを、ゲートが確かめる場面がその例です。
空欄はワークフローファイルの中にしかなく、確かめるにはそのファイルを読むしかありません([ADR-0033](adr/0033-the-one-variable-headsign-sets.md))。
...
制御グラフは、仕事が次にどこへ行くかだけを示します。
知識グラフではありません。
あなたのドメインについての事実は一つも持ちません。
実行トレースでもありません。
run が始まる前に書かれ、run 自身の履歴は `.headsign/log` に残ります。
...
このファイルは gitignore されていて捨ててよいものです。
まっさらにしたくなったら削除してください。

run と run のあいだに区切りは入りません。
run は必ず自分の `start` 行から始まるからです。
その行は、スクリプトが信頼してよい目印です。
イベント名は必ず 2 番目のフィールドにあり、`abort` の理由のような自由文は必ず `a=` と `i=` より後ろに来ます。
...
これは Claude Code から渡される hook payload のフィールド名であって、headsign が決めているものではありません。
このフィールド名を行に載せることで、ログ、headsign のソース、人が出力できる payload を同じ 1 語で追跡できます。
...
人間向けの、その失敗自身の行は、すでに `timed out after Ns` として所要時間を示します。
`retry` では RETRY 自身の `--- gate failed: … ---` 行です。
routed-fail では ADVANCE ブロックの `--- gate failed: … → routed to … ---` 行です。
そのため、そちらでは所要時間を繰り返しません。
...
グラフであること自体は手柄ではありません。
モデルが「終わった」と言うまで再投入するループに対して、グラフが何を足すのかを正直に採点しておきます。

- **独立した検証:あります。ただし、あなたのチェックが独立している範囲で。**
  遷移を決めるのはワークフロー定義に書かれたコマンドであって、作業しているエージェント自身の報告ではありません。
  レビューのフェーズなら、判定を読み取り専用の reviewer の手に委ねることもできます。
  それがどこまで届くのか、hard / semi / soft のゲートの話は [ADR-0007](adr/0007-verdict-authorship.md) にあります。
...
現在、この形には名前があります。
*graph engineering* は、[最初に書き留められた定義](https://www.drjoshcsimmons.com/writing/we-are-entering-the-graph-engineering-phase)では「エージェント的なシステムを、暗黙のループではなく明示的なグラフとして設計すること」です。
古いほうの名前との境界をこう引いています。
...
headsign は、この定義にある三つの語のうち一つを満たしません。
...
残る二つについて、headsign は定義に足りていないのではなく、定義に対して裏返っています。
...
判断はフェーズの中に置きます。
遷移はそれを置いてはいけない唯一の場所です。

同じ定義は、もう一つ求めています。
「人間をノードとして扱え。承認は、他のどんな能力とも同じだけの設計上の注意に値する」。
headsign に承認機能は無く、必要もありません。
人間のノードは、既に普通のフェーズだからです。
そのフェーズでは、人しか書かない決定ファイルをゲートが読みます。
[example.headsign/release.yaml](../example.headsign/release.yaml) の `approve` フェーズがその例です。
