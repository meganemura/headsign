...
終わらないのは `.headsign/log` です。入力した理由も、それ以前に記録されたものも残ります。`start` は消さずに追記するからです([ログの読み方](#ログの読み方))。
つまり意図的に run を終わらせても、失うのは run であって履歴ではありません。新しい `start` が丸ごと書き換えるのは `state.json` のほうです。
