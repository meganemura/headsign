...
`timeout:` で打ち切られた検査はそう言うので、たまたま同じだけ時間のかかった通常の失敗と読み違えることはありません。
...
委譲先か、人か、ビルドかを名指すか、名指すものが無いと気づいて進むかのどちらかです。
記録も読めるようになります。
log は note の 1 行目を持つので、待ち先を名指した中断と、名指していない中断とが、あとから区別できます。
...
上の 2 つの読み替えではなく、3 つ目のタイムスタンプです。
...
そこが上の `last moved:` との違いです。あちらは `start` と `next` のたびに、答えが何であれ刻まれます。
2 つを並べて読んでください。`entered:` はこのフェーズが何分続いているかを言い、`last moved:` は誰かが最後に run を触ったのがいつかを言います。
新しい `last moved:` の下に古い `entered:` があればリトライを重ねているフェーズで、両方古ければ誰も駆動していない run です。
経過時間は引き算で出してください。headsign は記録したとおりのタイムスタンプを出すだけで、自分では時間を計算しません([ADR-0031](adr/0031-when-the-run-entered-the-phase.md))。
